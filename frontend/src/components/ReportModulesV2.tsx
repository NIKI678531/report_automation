...
import { reportMonthName, reportPageEyebrow, reportProductTicker, type ModuleId } from "../reportModules";
...
function AnalyticsModule({ report, busy, run }: Omit<ModuleProps, "active">) {
  ...
  return <><ModuleHeading eyebrow={reportPageEyebrow("analytics", "Calculated outputs")} title="Final Analytics" description="Upload authoritative KPI, calendar and event observations; HSICS mapping comes from the report-date master." /><div className="dataset-slot-grid"><CsvDatasetUpload report={report} datasetType="fund_kpi_daily" busy={busy} run={run} /><CsvDatasetUpload report={report} datasetType="trading_calendar" busy={busy} run={run} /><CsvDatasetUpload report={report} datasetType="index_events" busy={busy} run={run} /><IndustryMasterStatus report={report} /></div><div className="analytics-grid"><section className="analytics-section"><SectionTitle index="01" title="Top 10 Index Constituents" /><table><tbody>{top10.map((row, index) => <tr key={`${String(row.issuer)}-${index}`}><th>{String(row.issuer)}</th><td>{percent(row.weight)}</td></tr>)}</tbody></table>{!top10.length && <EmptyData />}</section><section className="analytics-section"><SectionTitle index="02" title="Index Sectors Breakdown" /><div className="sector-list">{sectors.map((row) => <div key={String(row.sector)}><div><span>{String(row.sector)}</span><strong>{percent(row.weight)}</strong></div><i><b style={{ width: percent(row.weight) }} /></i></div>)}</div>{!sectors.length && <EmptyData />}</section><section className="analytics-section"><SectionTitle index="03" title={`Performers in ${monthName}`} /><div className="performer-columns"><PerformerList title="Top" data={top} /><PerformerList title="Bottom" data={bottom} /></div></section><section className="analytics-section"><SectionTitle index="04" title={`${productTicker} Portfolio Analysis`} /><dl className="portfolio-list">{portfolio.map((row) => <div key={String(row.label)}><dt>{String(row.label)}</dt><dd>{String(row.value)}</dd></div>)}</dl>{!portfolio.length && <EmptyData />}</section></div><FormulaStrip title="Analytics calculation set" formula="Weight ranking · HSICS aggregation · 1M performer ranking" detail="Every output is recalculated automatically when the required logical datasets form a valid snapshot." /></>;
}
