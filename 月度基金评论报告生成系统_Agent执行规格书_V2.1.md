...
| 安全区 | 内容不得进入页脚Logo/页码区；所有页保持一致左右边界和顶部基线；精确边距从黄金PDF测量后写入`design-tokens/3033-v1.json`，不得散落为代码常量 |
...
```text
tests/fixtures/3033_202606/reference.pdf
tests/fixtures/3033_202606/reference-pages/page-01.png ... page-04.png
tests/fixtures/3033_202606/reference-analysis.json
tests/fixtures/3033_202606/masks/                 # 仅存经批准的动态区域遮罩
artifacts/visual/pdf/{run_id}/actual-pages/
artifacts/visual/pdf/{run_id}/diff-pages/
artifacts/visual/html/{run_id}/desktop-mobile-print/
artifacts/visual/docx/{run_id}/word-to-pdf-pages/
artifacts/visual/{run_id}/manifest.json            # 输入、版本、字体、哈希、阈值与结果
```
...
```text
apps/web                  # React + TypeScript编辑器、预览与管理端
services/api              # Python + FastAPI业务API（/api/v1、OpenAPI）
services/worker           # 快照/新闻/AI/渲染任务
packages/domain           # 实体、状态机、权限与业务规则
packages/calculation      # 纯计算与公式版本
packages/connectors       # CDB/上传/新闻/AI适配器
packages/renderers        # PDF/HTML/DOCX
packages/design-tokens    # 3033模板令牌、字体与品牌资产清单
infra/                    # 容器、部署、监控和环境
migrations/               # 数据库迁移
...
| WP6-01 | 从3033黄金样例固化A4页面、字体、颜色、间距、页眉页脚、四页结构、共享DesignToken和规范HTML/CSS | design-tokens/3033-v1.json + render schema + 资产清单 | Marketing批准且三格式映射文档完成 |
